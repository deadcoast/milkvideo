...
class BatchProcessor:
    ...
    def process_audio_batch(self, urls: List[str], audio_format: str = "mp3", options: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        with Progress(
            ...
            for chunk_start in range(0, len(urls), chunk_size):
                ...
                for i, url in enumerate(chunk_urls):
                    ...
                    # Update progress description
                    progress.update(task, description=f"Active: {active_threads}/{max_concurrent}, Completed: {completed_count}/{len(urls)}")
    ...
    def process_audio_batch_with_limits(self, urls: List[str], 
        ...
        with Progress(
            ...
            task = progress.add_task(f"Extracting audio from {len(urls)} videos (max {max_concurrent} concurrent)", total=len(urls))
