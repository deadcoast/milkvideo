...
class InputHandler:
    ...
    def parse_batch_file(self, file_path: Path) -> List[str]:
        ...
        try:
            ...
            raise ValueError(f"Failed to read batch file: {e}")
    ...
    def parse_quality_setting(self, quality: str) -> Optional[str]:
        ...
        valid_qualities = [
            'best', 'worst', '720p', '1080p', '480p', '360p',
            'audio_only', 'video_only', 'bestvideo', 'bestaudio'
        ]
    ...
    def parse_number_input(self, input_text: str, min_value: Optional[int] = None, 
    ...
    def parse_time_input(self, time_input: str) -> Optional[int]:
        ...
        # Hours
        hour_match = re.search(r'(\d+)h', time_input)
        if hour_match:
            total_seconds += int(hour_match.group(1)) * 3600
        
        # Minutes
        minute_match = re.search(r'(\d+)m', time_input)
        if minute_match:
            total_seconds += int(minute_match.group(1)) * 60
        
        # Seconds
        second_match = re.search(r'(\d+)s', time_input)
        if second_match:
